...
for j in range(10):
    ...
    pendulumoc.recmat_init_step(horizon,-1)
    current_parameter = np.random.randn(pendulumoc.n_auxvar)
    max_iter = 2e5
    # iteration
    for k in range(int(max_iter)):
        # one iteration of PDP
        loss, dp = pendulumoc.recmat_step(ini_state, horizon, current_parameter)
        # update
    ...
    sol = pendulumoc.recmat_unwarp(ini_state, horizon, current_parameter)
